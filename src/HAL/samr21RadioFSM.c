...
static const fsmItem s_fsmTxItemTable[]={
    ...
    /*------------*/fsm_func_samr21RadioJobCleanup}, 
                    ...
                    {RADIO_JOB_STATE_TX_SENDING,        
                        RADIO_SOFTEVENT_ACK_REQUESTED,  
                            RADIO_JOB_STATE_TX_SENDING_ACK_REQUESTED, 
                    NULL}, 

                        {RADIO_JOB_STATE_TX_SENDING_ACK_REQUESTED,        
                            RADIO_EVENT_IRQ_TRX_END,  
                                RADIO_JOB_STATE_TX_WAITING_FOR_ACK, 
    ...
    /*----------------*/fsm_func_samr21RadioJobCleanup},
                            ...
                            {RADIO_JOB_STATE_TX_WAITING_FOR_ACK,        
                                RADIO_EVENT_TIMER_TRIGGER,  
                                    RADIO_JOB_STATE_TX_EVAL_RETRANSMISSIOM, 
    ...
    /*------------------------*/fsm_func_samr21RadioJobCleanup},
                            ...
                            {RADIO_JOB_STATE_TX_WAITING_FOR_ACK,        
                                RADIO_EVENT_IRQ_RX_START,  
                                    RADIO_JOB_STATE_TX_RECEIVING_ACK, 
                            fsm_func_samr21RadioAckReceptionStarted},

                                {RADIO_JOB_STATE_TX_RECEIVING_ACK,        
                                    RADIO_EVENT_IRQ_TRX_END,  
                                        RADIO_JOB_STATE_TX_EVAL_ACK, 
...
#define RADIO_JOB_STATE_TX_LAST RADIO_JOB_STATE_TX_DONE

static const fsmItem s_fsmRxItemTable[]={

        {RADIO_JOB_STATE_RX_IDLE,                
            RADIO_EVENT_IRQ_RX_START,     
                RADIO_JOB_STATE_RX_LIVE_MSG_PARSER,      
        fsm_func_samr21RadioLiveRxParser},

            {RADIO_JOB_STATE_RX_LIVE_MSG_PARSER,                
                RADIO_SOFTEVENT_ACK_REQUESTED,     
                    RADIO_JOB_STATE_RX_SENDING_ACK,      
            fsm_func_samr21RadioSendAck},

                {RADIO_JOB_STATE_RX_SENDING_ACK,                
                    RADIO_EVENT_IRQ_TRX_END,     
                        RADIO_JOB_STATE_RX_DONE,      
    /*--------*/fsm_func_samr21RadioJobCleanup},

            {RADIO_JOB_STATE_RX_LIVE_MSG_PARSER,                
                RADIO_EVENT_TIMER_TRIGGER,     
                    RADIO_JOB_STATE_IDLE,      
...
#define RADIO_JOB_STATE_RX_LAST RADIO_JOB_STATE_RX_DONE

static const fsmItem s_fsmEdItemTable[]={

        {RADIO_JOB_STATE_ED_READY,                
...
#define RADIO_JOB_STATE_ED_LAST RADIO_JOB_STATE_ED_DONE
...
void samr21RadioFsmHandleEvent(RadioEvent event)
{
    ...
    if (*(currentStatePtr) < MARKER_RADIO_JOB_STATES_END_RX){
beginFsmRxHandler:
        for (uint8_t i = 0; s_fsmRxItemTable[i].state != RADIO_JOB_STATE_RX_LAST; i++)
        {		
            ...
            //Update State
            if(*(currentStatePtr) < MARKER_RADIO_JOB_STATES_BEGINN_OTHER_DONE){
    ...
    if (*(currentStatePtr) < MARKER_RADIO_JOB_STATES_END_TX){
beginFsmTxHandler:
        for (uint8_t i = 0; s_fsmTxItemTable[i].state != RADIO_JOB_STATE_TX_LAST; i++)
        {		
    ...
    if (*(currentStatePtr) < MARKER_RADIO_JOB_STATES_END_ED){
beginFsmEdHandler:
        for (uint8_t i = 0; s_fsmEdItemTable[i].state != RADIO_JOB_STATE_ED_LAST; i++)
        {		
