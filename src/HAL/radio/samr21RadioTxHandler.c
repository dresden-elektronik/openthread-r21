...
bool samr21RadioTxSetup()
{
    ...
    if(
        otMacFrameIsSecurityEnabled(&s_txFrame) 
        && otMacFrameIsKeyIdMode1(&s_txFrame) 
        && !s_txFrame.mInfo.mTxInfo.mIsARetx
        && !s_txFrame.mInfo.mTxInfo.mIsHeaderUpdated
    ){
        __disable_irq();
...
void samr21RadioTxEvalCCA()
{
    ...
    if(
        s_numCsmaBackoffs < s_txFrame.mInfo.mTxInfo.mMaxCsmaBackoffs
        && s_txFrame.mInfo.mTxInfo.mCsmaCaEnabled
    ){
        uint32_t backoffTime_us = 
            ( ( samr21TrxGetRandomByte() >> (8 - min(g_csmaBackoffExponentMin+s_numCsmaBackoffs, g_csmaBackoffExponentMax) ) ) - 1 )
            * ( 20 * AT86RF233_SYMBOL_TIME_us ) 
        ;
        s_numCsmaBackoffs++;
        ...
        for(uint8_t i = 0; i<AES_BLOCK_SIZE; i++){
            ...
            if(securityLevel >= 0b100){
                samr21TrxSpiTransceiveByteRaw(
                    ...
                    ^ s_txEncryptionMask[1+numProcessedEncryptionBlocks][i]
                );
            } else {
                samr21TrxSpiTransceiveByteRaw(
                    s_txFrame.mPsdu[s_numBytesPdsuUploaded++]
...
/******************************Authenicate Payload and upload (encrypted) MIC (CBC-MAC)************************************/  
    if(footerLenght > 2){//MIC requested
...
//Dowload LQI, RSSI and CRC Check (r21 Datasheet 35.3.2 -  Frame Buffer Access Mode)
    ...
    if(
        otMacFrameIsAck(&s_txAckFrame)
        && rxStatus.bit.crcValid
        && otMacFrameGetSequence(&s_txFrame) == otMacFrameGetSequence(&s_txAckFrame)
    ){
