...
static void samr21RadioTxUploadPayload()
{
    ...
    while (s_currentTransmissionSecurity.ctr.numEncryptionBlocks > s_currentTransmissionSecurity.ctr.numProcessedEncryptionBlocks)
    {
        ...
        if (s_currentTransmissionSecurity.needsEncryption)
        {
            ...
            s_currentTransmissionSecurity.ctrBlock[(AES_BLOCK_SIZE - 1)]++;
            // start uploading next AES Block, while reading the result of the last Block
            samr21RadioAesEcbEncrypt(s_currentTransmissionSecurity.ctrBlock, s_currentTransmissionSecurity.pendingAesResultBuffer);

            s_currentTransmissionSecurity.pendingAesResultBuffer =
                s_currentTransmissionSecurity.encryptionBlocks[s_currentTransmissionSecurity.ctrBlock[(AES_BLOCK_SIZE - 1)]];
        }
        ...
        for (uint8_t i = 0; i < AES_BLOCK_SIZE; i++)
        {
            ...
            if (s_currentTransmissionSecurity.needsEncryption)
            {
                samr21TrxSpiTransceiveByteRaw(
                    s_txFrame.mPsdu[s_currentTransmission.numPsduBytesUploaded++] ^ s_currentTransmissionSecurity.encryptionBlocks[s_currentTransmissionSecurity.ctr.numProcessedEncryptionBlocks][i]);
            }
            else
            {
                samr21TrxSpiTransceiveByteRaw(
    ...
    if (s_currentTransmissionSecurity.needsEncryption)
    {
        ...
        s_currentTransmissionSecurity.ctrBlock[(AES_BLOCK_SIZE - 1)] = 0; ////The Nonce for MIC encryption has the CTR value 0
        samr21RadioAesEcbEncrypt(s_currentTransmissionSecurity.ctrBlock, s_currentTransmissionSecurity.pendingAesResultBuffer);
        s_currentTransmissionSecurity.pendingAesResultBuffer =
            s_currentTransmissionSecurity.encryptionBlocks[s_currentTransmissionSecurity.ctrBlock[(AES_BLOCK_SIZE - 1)]];
    }
    ...
    s_txStatus = TX_STATUS_SENDING_UPLOADED_PAYLOAD;
}
...
static void samr21RadioTxCalcMic()
{
    ...
    if (s_txStatus != TX_STATUS_SENDING_BUSY_MIC)
    {
        ...
        s_currentTransmissionSecurity.cbcBlock[0] =
            (s_currentTransmissionSecurity.headerLen != 0 ? 0b01000000 : 0x0) | (((s_currentTransmissionSecurity.micSize - 2) >> 1) << 3) | 1; // L always 2
        s_currentTransmissionSecurity.cbcBlock[14] = 0;                                                                                        // always 0 cause maxlenght is 127
        s_currentTransmissionSecurity.cbcBlock[15] = s_currentTransmissionSecurity.payloadLen;
    ...
    // Still Rounds to go
    if (
        s_currentTransmissionSecurity.cbc.numProcessedHeaderBytes < s_currentTransmissionSecurity.headerLen || s_currentTransmissionSecurity.cbc.numProcessedPayloadBytes < s_currentTransmissionSecurity.payloadLen)
    {
...
fillNextBlock:
    ...
    if (s_currentTransmissionSecurity.cbc.numProcessedHeaderBytes < s_currentTransmissionSecurity.headerLen)
    {
        while (s_currentTransmissionSecurity.cbc.currentBlockLen < AES_BLOCK_SIZE)
        {
            s_currentTransmissionSecurity.cbcBlock[s_currentTransmissionSecurity.cbc.currentBlockLen++] =
                (s_currentTransmissionSecurity.cbc.numProcessedHeaderBytes < s_currentTransmissionSecurity.headerLen ? s_currentTransmissionSecurity.header[s_currentTransmissionSecurity.cbc.numProcessedHeaderBytes++]
                                                                                                                     : 0x00);
        }
        return;
    }
    ...
    if (s_currentTransmissionSecurity.cbc.numProcessedPayloadBytes < s_currentTransmissionSecurity.payloadLen)
    {
        ...
        {
            s_currentTransmissionSecurity.cbcBlock[s_currentTransmissionSecurity.cbc.currentBlockLen++] =
                (s_currentTransmissionSecurity.cbc.numProcessedPayloadBytes < s_currentTransmissionSecurity.payloadLen ? s_currentTransmissionSecurity.payload[s_currentTransmissionSecurity.cbc.numProcessedPayloadBytes++]
                                                                                                                       : 0x00);
        }
        return;
    }
...
static void samr21RadioTxUploadMic()
{
    ...
    samr21RadioAesCbcEncrypt(NULL, AES_BLOCK_SIZE, s_currentTransmissionSecurity.cbcBlock); // TODO REMOVE USELESS RUN REQUEST
...
static void samr21RadioTxStartTransmission()
{
    ...
    // Start Transmission in advance, there is some spare time while the Preamble and SFD is transmitted
    ...
    case 0b10:
        s_currentTransmissionSecurity.micSize = 8;
        break;
    ...
    if (securityLevel)
    {
        ...
        if (s_currentTransmissionSecurity.needsEncryption)
        {
            ...
            samr21RadioAesEcbEncrypt(s_currentTransmissionSecurity.ctrBlock, NULL);
    ...
    // Only Executed if Auth but no Encryption is needed!
    // Otherwise this gets called by a timer after the first AES-Operation finishes
    // Upload the Payload next.
